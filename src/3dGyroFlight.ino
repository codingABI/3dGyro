/*
 * Project: 3dGyroFlight
 * Description: 3d object controlled by a gyro sensor MPU6050 on an arduino UNO/NANO with a SSD1306 OLED 128x64 pixel display.
 * License: MIT License
 * 
 * created by codingABI https://github.com/codingABI/3dGyroFlight
 * 
 * History:
...
void loop(void) {
  ...
  static signed char deltaPosition[3] = {0,0,0}; // 3D object movement relative from origin
  ...
  if (dmpReady) {
    ...
    // Modul is upsidedown in my project
  ...
  if (millis() - lastMS > 1000) {
    // Move object in allowed area once per second along vector P1-P5
    if (abs((int) deltaPosition[X] + (pointsTransformed3d[P1][X]-pointsTransformed3d[P5][X]) / 10) < 34) deltaPosition[X] += (pointsTransformed3d[P1][X]-pointsTransformed3d[P5][X]) / 10;
    if (abs((int) deltaPosition[Y] + (pointsTransformed3d[P1][Y]-pointsTransformed3d[P5][Y]) / 10) < 34) deltaPosition[Y] += (pointsTransformed3d[P1][Y]-pointsTransformed3d[P5][Y]) / 10;
    
    for (byte i=0;i<MAXPOINTS;i++) {      
      pointsTransformed3d[i][Y] += deltaPosition[Y];
      pointsTransformed3d[i][X] += deltaPosition[X];
    }
  }
